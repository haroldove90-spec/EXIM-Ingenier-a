  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex border-b border-gray-100">
        ...
        >
          <Users2 className="h-4 w-4" /> Directorio de Clientes (CRM)
        </button>
        <button
        ...
        >
          <Building className="h-4 w-4" /> Base de Datos de Proveedores
        </button>
      </div>
          ...
          <div className="space-y-2.5">
            {activeTab === "clients"
              ? clients.map((c) => (
                  ...
                  >
                    <span className="font-mono text-[10px] font-bold text-gray-400 block">{c.id}</span>
                    <h3 className="text-xs font-bold text-gray-900 mt-1 line-clamp-1">{c.name}</h3>
                    <p className="text-[10px] text-gray-400 truncate mt-0.5">{c.fiscalAddress}</p>
                    {c.contacts.length > 0 && (
              ...
              : providers.map((p) => (
                  ...
                  >
                    <span className="font-mono text-[10px] font-bold text-gray-400 block">{p.id}</span>
                    <h3 className="text-xs font-bold text-gray-900 mt-1 line-clamp-1">{p.name}</h3>
                    <span className="text-[9px] bg-slate-100 text-slate-700 font-bold px-1.5 py-0.2 rounded mt-1.5 inline-block">
                      {p.category}
                    </span>
                  </button>
                ))}
          ...
          {activeTab === "clients" && selectedClient ? (
            <div className="glass-card p-5 space-y-6">
              ...
              <div className="border-b border-gray-50 pb-4">
                <span className="font-mono text-xs font-bold text-blue-600 block">{selectedClient.id}</span>
                <h2 className="text-sm font-bold text-gray-900 mt-0.5">{selectedClient.name}</h2>
                <div className="mt-2 text-xs font-serif text-gray-500">
                  <span className="font-bold text-gray-700 font-sans">RFC:</span>{" "}
                  <span className="font-mono">{selectedClient.rfc}</span>
                </div>
                <div className="mt-1 text-xs text-stone-500">
                  <span className="font-bold text-gray-700">Fiscal address:</span> {selectedClient.fiscalAddress}
                </div>
              </div>

              {/* Linked contacts info list */}
              <div>
                <span className="text-xs font-extrabold text-gray-400 uppercase tracking-wider block mb-2.5">
                  Contactos Clave de Enlace
                </span>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {selectedClient.contacts.map((contact, idx) => (
                    <div key={idx} className="rounded-lg bg-gray-50 p-3.5 border border-gray-100 space-y-2 text-xs">
                      <div>
                        <span className="font-bold text-gray-900 block">{contact.name}</span>
                        <span className="text-[10px] text-indigo-600 font-bold uppercase">{contact.role}</span>
                      </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-xs">
                  {selectedClient.serviceAddresses.map((addr, idx) => (
                    <div key={idx} className="rounded-lg p-3 bg-indigo-50/30 border border-indigo-100 flex items-start gap-2.5">
                      <MapPin className="h-4 w-4 text-indigo-500 shrink-0 mt-0.5" />
                      <div>
                        <strong className="text-gray-800 font-bold block">{addr.name}</strong>
                        <span className="text-stone-500">{addr.address}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="border-t border-gray-100 pt-5 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  {/* Past quotes stats */}
                  <div className="rounded-lg bg-slate-50 p-3.5 border border-gray-100">
                    ...
                    {clientQuotes.length > 0 ? (
                      <div className="space-y-1 text-[11px]">
                        {clientQuotes.map((q) => (
                          <div key={q.id} className="flex justify-between font-medium">
                            <span className="font-mono text-gray-500">{q.id}</span>
                            <span className="font-bold font-mono">${q.total.toLocaleString("es-MX")}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <span className="text-[10px] text-gray-400 italic">No hay propuestas previas</span>
                    )}
                  </div>
                      ...
                      <div className="space-y-1 text-[11px]">
                        {clientProjects.map((p) => (
                          <div key={p.id} className="flex justify-between font-medium">
                            <span className="font-mono text-gray-500 font-bold uppercase">{p.id}</span>
                            <span className="truncate max-w-[80px] text-stone-600">{p.name}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <span className="text-[10px] text-gray-400 italic">Sin obras activas en planta</span>
                    )}
                  </div>
                  ...
                  <div className="rounded-lg bg-slate-50 p-3.5 border border-gray-100">
                    ...
                    {clientOrders.length > 0 ? (
                          ...
                          <div key={o.id} className="flex justify-between font-medium">
                            ...
                            <span
                              className={`text-[9px] px-1 rounded font-bold ${
                                o.status === "Finalizado" ? "text-green-700 bg-green-50" : "text-gray-500"
                              }`}
                            >
                              {o.status}
                            </span>
                      ...
                      <span className="text-[10px] text-gray-400 italic">No hay tickets de guardia</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="glass-card p-5 space-y-5">
              <div className="border-b border-gray-50 pb-4">
                <span className="font-mono text-xs font-bold text-indigo-600 block">{selectedProvider.id}</span>
                <h2 className="text-sm font-bold text-gray-900 mt-0.5">{selectedProvider.name}</h2>
                <span className="text-[10px] bg-indigo-50 text-indigo-700 font-bold px-2 py-0.5 rounded mt-2.5 inline-block uppercase">
              ...
              <div className="space-y-3.5 text-xs text-gray-600">
                <div>
                  <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Identificación RFC</span>
                  <span className="font-bold text-gray-900 font-mono">{selectedProvider.rfc}</span>
                </div>
                <div>
                  <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Teléfono de Suministro</span>
                  <span className="font-bold text-gray-900">{selectedProvider.phone}</span>
                </div>
                <div>
                  <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Correo de compras/pedidos</span>
                  <span className="font-bold text-indigo-600 leading-3">{selectedProvider.email}</span>
                </div>
                <div>
                  <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Dirección Comercial</span>
                  <span className="font-bold text-gray-800">{selectedProvider.address}</span>
                </div>
              </div>
            </div>
          ) : (
